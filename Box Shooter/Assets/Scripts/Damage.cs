...
public class Damage : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider collision)                     // used for things like bullets, which are triggers.  
    {
        if (damageOnTrigger)
        {
            if (this.tag == "Projectile" && collision.gameObject.tag == "Player")   // if the player got hit with it's own bullets, ignore it
                return;
            ...
            {   // if the hit object has the Health script on it, deal damage
                ...
                // case for player object
                if (collision.gameObject.GetComponent<PlayerHealth>() != null)


                {   // if the hit object has the PlayerHealth script on it, deal damage
                    collision.gameObject.GetComponent<PlayerHealth>().ApplyDamage(damageAmount);
